...
class UserModel:
    ...
    def update(self, user_name, user_id):
        cursor = self.connection.cursor()
        cursor.execute(
            '''UPDATE users SET user_name = '{}' WHERE id = {}'''.format(
    ...
    def unfollow(self, user_id, community_id):
        ...
        if str(community_id) in communities:
            ...
            cursor.execute(
                '''UPDATE users SET communities = '{}' WHERE id = {}'''.format(
                    ' '.join(communities), str(user_id)))
        cursor.close()
        self.connection.commit()
...
class NewsModel:
    ...
    def insert(self, community, title, hashtag, content, image, date, user_id):
        ...
        cursor.execute('''INSERT INTO news
                          (community, title, hashtag, content, image, date,
                          user_id)
                          VALUES (?,?,?,?,?,?,?)''', (
        community, title, hashtag, content, image, date, str(user_id)))
        cursor.close()
        self.connection.commit()
    ...
    def update(self, news_id, title=None, hashtag=None, content=None):
        ...
        if title:
            cursor.execute(
                '''UPDATE news SET title = '{}' WHERE id = {}'''.format(title,
                                                                str(news_id)))
        if content:
            cursor.execute(
                '''UPDATE news SET content = '{}' WHERE id = {}'''.format(
                    content, str(news_id)))
        if hashtag:
            cursor.execute(
                '''UPDATE news SET hashtag = '{}' WHERE id = {}'''.format(
                    hashtag, str(news_id)))
        cursor.close()
        self.connection.commit()
...
class CommunityModel:
    ...
    def get(self, community_id):
        cursor = self.connection.cursor()
        cursor.execute("SELECT * FROM communities WHERE id = ?",
                       (str(community_id),))
        row = cursor.fetchone()
        return row
    ...
    def get_name(self, community_name):
        cursor = self.connection.cursor()
        cursor.execute("SELECT * FROM communities WHERE title = ?",
                       (str(community_name),))
        row = cursor.fetchone()
        return row
    ...
    def update(self, community_id, title=None, bio=None):
        ...
        if title:
            cursor.execute(
                '''UPDATE communities SET title = '{}' WHERE id = {}'''.format(
                    title, str(community_id)))
        if bio:
            cursor.execute(
                '''UPDATE communities SET bio = '{}' WHERE id = {}'''.format(
                    bio, str(community_id)))
        cursor.close()
        self.connection.commit()
    ...
    def unfollow(self, user_id, community_id):
        ...
        if str(user_id) in users:
            ...
            cursor.execute(
                '''UPDATE communities SET users = '{}' WHERE id = {}'''.format(
                    ' '.join(users), str(community_id)))
        cursor.close()
        self.connection.commit()
    ...
    def delete(self, community_id):
        cursor = self.connection.cursor()
        cursor.execute('''DELETE FROM communities WHERE id = ?''',
                       (str(community_id),))
        cursor.close()
        self.connection.commit()
